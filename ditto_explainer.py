...
from ditto_helper import predict_ditto

from tqdm import tqdm
from torch.utils import data

class DittoExplainer(object):
    # The explainer class
    def __init__(self,
                 model,
                 configs,
    ...
    def _explain_permutation(self, instance, cands4attrs, k):
        ...
        for indices in product(*perms):
            ...
            if num == 0:
                continue
            sample = self._format_ditto_instance(cands4attrs,indices,instance)
    ...
    def _explain_min_attr(self, instance, cands4attrs, k):
        ...
        for num in dimnums:
            ...
            for item in tmp_dict[num]:
                sample = self._format_ditto_instance(cands4attrs,item,instance)
                predict, logit = self._eval_model([sample])
                eval_cnt += 1
                if str(predict[0]) != str(instance[2]): # result is flipped
                    candidates.append((sample,num,max(logit[0][0],logit[0][1]),indices))
    ...
    def _eval_model(self, samples, prob=True):
        ...
        results, logits = predict_ditto(samples, self.ditto_configs, self.model, batch_size=len(samples))
        if prob == True:
            return results, logits
        else:
            return results